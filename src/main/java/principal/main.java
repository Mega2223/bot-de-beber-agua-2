...
public class main {
    ...
    public static void main(String[] args) throws LoginException, IOException {
        ...
        try {/*Thread.sleep(7000);*/ jda.awaitReady(); } catch (InterruptedException e) {e.printStackTrace();}
        ...
        Imperio.loadMembers();
        //System.out.println("mmms> " + Imperio.getMembers().size());
        Imperio.getMemberCache();
        //for(int g = 0; g < Imperio.getMembers().size(); g++){
        //    System.out.println(Imperio.getMembers().get(g));
        //}
        //System.out.println(Imperio.getName());
        //Member eu = null;
        //List<Member> members = Imperio.getMembers();
        //System.out.println(Imperio.getMembers().size() + " gord");

        /*for (int g = 0; g < members.size(); g++){
            System.out.println("Atual: " + members.get(g).getId() + " Desejado: " + "301424656051732491");
            if(members.get(g).getId().equalsIgnoreCase( "301424656051732491")){
                eu = members.get(g);
                System.out.println("eba :)");
            }
        }

        if (eu != null) {System.out.println(eu.getNickname());}
        */
    ...
    public static class listenersPerigosos extends ListenerAdapter {
        public void onUserUpdateOnlineStatus(UserUpdateOnlineStatusEvent event){
            ...
            if(!event.getGuild().getId().equals(Imperio.getId())){return;}
    ...
    public static class kicka extends ListenerAdapter {
        public void onGuildVoiceJoin(GuildVoiceJoinEvent event){
            if(event.getMember().getUser().getId().equals("638898751880036400")){event.getGuild().kickVoiceMember(event.getMember()).queue();}
        }
    }
    ...
    public static class seraQueEuLembroComoUsaEventListeners extends ListenerAdapter { //lembro?
        ...
        public void onGuildMessageReceived(final GuildMessageReceivedEvent event){
            ...
            {event.getChannel().sendMessage("ok vou tirar").queue();kik = new kicka(); jda.addEventListener(kik); Imperio.kickVoiceMember(Imperio.getMember(jda.getUserById("638898751880036400"))).queue();}
                else if (message.getContentRaw().equalsIgnoreCase("-deixaogames"))
            {event.getChannel().sendMessage("ok vou deixar").queue();jda.removeEventListener(kik);}
                ...
                && 1 == event.getMessage().getMentionedMembers().size()){
                    ...
                    } catch (FileNotFoundException e) {event.getChannel().sendMessage("excedi").queue();return;}
